...
def game(maxMoves):
        ...
        def findDensity(self, pointRange, pointWithin=None):
            ...
            for x1 in range(0, self.columns - pointRange):
                for y1 in range(0, self.rows - pointRange):
                    ...
                    currentDensity = 0
                    currentPoints = {}
            print(self.densityField)

        def determine_distribution(self):
            average = sum(self.densityField.keys()) / len(self.densityField.keys())
            largest = max(self.densityField.keys())
            if largest - average > highest * len(self.densityField[list(self.densityField.keys())[0]]) / 2:
    ...
    class Robot:
        ...
        def move(self, movementType, iterations=1):
            if movementType == "Astar":
                ...
                self.x, self.y = self.path[0][0], self.path[0][1]
                self.path.remove(self.path[0])
            elif movementType == "combined":
                if self.grid.determine_distribution() == "gradient":
                    if len(self.path) == 1:
                        self.path = self.Dijkstra(5)
    ...
    grid = Grid(rows, columns, distribution="gradient")
    grid.findDensity(5)
    ...
    while running:
        ...
        if not pause:
            robot3.move("combined", iterations=4) # moves bot
